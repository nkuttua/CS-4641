...
class RandomForest(object):
    ...
    def plot_feature_importance(self, data_train):
        ...
        feat = self.feature_indices[1]
        used = labels[feat]
        featI = self.decision_trees[1].feature_importances_
        featI = np.delete(featI, np.where(featI == 0))
        used = np.delete(used, np.where(featI == 0))
        sorted = np.argsort(featI)
        sorted = np.flip(sorted)
        plt.bar(range(featI.shape[0]), featI[sorted], tick_label = used[sorted])
