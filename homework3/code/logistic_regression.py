...
class LogisticRegression(object):
    ...
    def predict_probs(self, x_aug: np.ndarray, theta: np.ndarray) -> np.ndarray: # [5pts]
        ...
        """
        return 1 / (1 + np.exp(-np.dot(x_aug, theta)))
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        """
        return 1 / (x_aug.shape[0]) * np.dot(x_aug.T, h_x - y)
    ...
    def evaluate(self, x: np.ndarray, y: np.ndarray, theta: np.ndarray) -> Tuple[float, float]: # [5pts]
        ...
        """
        loss = self.loss(y, self.predict_probs(self.bias_augment(x), theta))
        accuracy = self.accuracy(y, self.predict_labels(self.predict_probs(self.bias_augment(x), theta)))
        return loss, accuracy

    def fit(self,x_train: np.ndarray,y_train: np.ndarray,x_val: np.ndarray,y_val: np.ndarray,lr: float,epochs: int,) -> Tuple[np.ndarray, List[float], List[float], List[float], List[float], List[int]]: # [5pts]
        ...
        epsilon = 1e-15
        y_pred = self.sigmoid(np.dot(x_train, theta))
        ll = - (y_train * np.log(y_pred) + (1 - y_train) * np.log(1 - y_pred)).mean()
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                self.update_evaluation_lists(x_train, y_train, x_val, y_val, theta, loss)
